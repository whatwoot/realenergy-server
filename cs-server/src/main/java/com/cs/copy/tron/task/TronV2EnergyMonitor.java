package com.cs.copy.tron.task;

import cn.hutool.core.util.HexUtil;
import cn.hutool.json.JSONArray;
import cn.hutool.json.JSONObject;
import com.cs.copy.tron.api.entity.TronEnergyRentalPrice;
import com.cs.copy.tron.api.service.TronEnergyRentalPriceService;
...
import com.cs.copy.tron.service.TronV2Client;
import com.cs.copy.tron.service.impl.EnergyAccountServiceImpl;
import com.cs.copy.tron.service.impl.EnergyRentalServiceImpl;
...
//    private long energyRatio;
    ...
    private void processTransaction(JSONObject tx) {
        try {
            ...
            String depositHex = tronClient.toHex(this.depositAddress);
...
//            // 检查最小金额
//            if (amount < minAmount) {
//                log.warn("转账金额过小，忽略: {} from {}", amount, fromAddress);
//                return;
//            }
            ...
            log.info("收到租用申请: {} 金额: {} SUN, 匹配档位能量: {}",
                    fromAddress, amount, rental.getEnergyAmount());
    ...
    private void executeRental(EnergyRental rental) {
        ...
        long rentalEnergyTrx = (long)Math.ceil(priceInfo.getEnergyAmount()/energyPricePerTrx*1e6);
        rental.setEnergyAmount(priceInfo.getEnergyAmount()); // 使用价格档位的能量数量
    ...
    public void monitorExpiration() {
        ...
        while (true) {
            ...
            if (dbRental == null || dbRental.getStatus() != 1) {
                continue;
            }
     ...
     */
    @Scheduled(fixedRate = 60000)
    public void monitorPendingRentals() {
        long now = System.currentTimeMillis();
