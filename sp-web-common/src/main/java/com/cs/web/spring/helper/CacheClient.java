...
public class CacheClient {
     ...
     * 缓存重建互斥key
     */
    private static final String LOCK_MUTEX = "lock:mutex:";

    /**
     * 缓存重建线程池
     */
    public static final ExecutorService CACHE_REBUILD_EXECUTOR = new ScheduledThreadPoolExecutor(4,
            new BasicThreadFactory.Builder().namingPattern("cache-client-pool-%d").daemon(true).build());
    ...
    public <T, ID> T handleCacheBreakdown(String keyPrefix, ID id, Class<T> type,
        ...
        String lockKey = LOCK_MUTEX + keyPrefix + id;
    ...
    public boolean tryLock(String key) {
        return tryLock(key, LOCK_TTL, TimeUnit.SECONDS);
    }

    public boolean tryLock(String key, long timeout, TimeUnit unit) {
        Boolean flag = stringRedisTemplate.opsForValue().setIfAbsent(key, "", timeout, unit);
        // 拆箱要判空，防止NPE
    ...
    public void withLock(String key, Runnable task) {
        withLock(key, LOCK_TTL, task);
    }

    public void withLock(String key, long expireTime, Runnable task) {
        boolean b = tryLock(key, expireTime, TimeUnit.SECONDS);
        WebAssert.expect(b, "chk.common.lockLimit");
        try {
            task.run();
        } finally {
            unlock(key);
    ...
    public <T> T withLock(String key, Supplier<T> task) {
        return withLock(key, LOCK_TTL, task);
    }

    public <T> T withLock(String key, long expireTime, Supplier<T> task) {
        boolean b = tryLock(key, expireTime, TimeUnit.SECONDS);
        WebAssert.expect(b, "chk.common.lockLimit");
        try {
            return task.get();
