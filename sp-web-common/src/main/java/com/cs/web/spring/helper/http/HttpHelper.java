package com.cs.web.spring.helper.http;
...
public class HttpHelper {
    ...
    private String execute(Request request) {
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            log.error("[{}] Http-fail | URL: {} | Error: {}", requestId, request.url(), e.getMessage());
            throwBizException("thd.http.fail");
        }
        return null;
    }
    ...
    private String handleResponse(Response response) throws IOException {
        if (!response.isSuccessful()) {
            ...
            err.put("code", response.code());
            err.put("msg", response.message());
            err.put("data", errorBody);
            throw new IOException(err.toJSONString());
        }
